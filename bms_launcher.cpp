...
int main() {
    ...
    if (!fileExists(hl2ExePath)) {
        MessageBoxA(NULL, "Error: bms.exe not found. Please place this file in the same directory as bms.exe. It seems you did not do that.", "HL2 Launcher Error", MB_OK | MB_ICONERROR);
        return 1;
    }
	...
	// Black Mesa launch command line arguments, -disabledvk and -forceuid3d9 are required or else the game will crash on launch. Thanks, crowbar collective.
    std::wstring commandLine = L"bms.exe -game bms -disabledxvk -forceuid3d9";
